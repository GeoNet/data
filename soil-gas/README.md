# Soil Gas
Soil gas data are collected manually by the Volcano Monitoring Group (VMG) at [Earth Sciences New Zealand (ESNZ)](https://www.earthsciences.nz/) to [monitor gas discharge rate through the ground in volcanic and geothermal areas](https://www.geonet.org.nz/volcano/how).
...
Each file in this repository contains the data collected from one survey of one volcanic area. Files are in [CSV (comma-separated values)](https://en.wikipedia.org/wiki/Comma-separated_values) format.
...
## Supporting Maps
To improve understanding of the area covered by CSV files, they are each accompanied by a map file showing the distribution of observation points. Maps use an [OpenStreetMap](https://www.openstreetmap.org/) basemap, show observation points as red-coloured circles, and a map scale, in units of metres (m) or kilometres (km), in the top right. All maps are [PNG](https://en.wikipedia.org/wiki/PNG) format files. Maps show the locations using the `NZTM-mE` and `NZTM-mN` fields. This normally implies CO<sub>2</sub> flux data at these points. To understand if CH<sub>4</sub>, H<sub>2</sub>S, or ground temperature data are present, refer to the corresponding CSV file.
